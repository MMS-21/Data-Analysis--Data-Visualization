...
The final output, integrated_customer_analysis.csv, merges all derived metrics RFM segment, predicted CLV, churn probability, and risk category—into a single source of truth for downstream business intelligence and campaign systems.
...
├── data/
│   ├── raw/
│   │   └── <source_data_file>.csv  # E-commerce transactional data (not provided, but inferred)
├── notebooks/
│   ├── 01_EDA.ipynb                # Exploratory Data Analysis
│   ├── 02_rfm_clv_analysis.ipynb   # RFM Segmentation and CLV Modeling
│   └── 03_churn_analysis.ipynb     # Churn Labeling and Prediction Modeling
├── reports/
│   ├── segment_summary.csv         # Summary statistics for each RFM segment
│   ├── rfm_clv_analysis.csv        # RFM scores and predicted CLV for all customers
│   ├── churn_risk_scores.csv       # Churn probability and risk category for all customers
│   ├── high_value_customers.csv    # List of 'Champions' and other high-scoring customers
│   └── integrated_customer_analysis.csv  # Final merged output
├── src/
│   ├── clv.py                      # Functions for RFM, CLV modeling (inferred)
│   └── churn.py                    # Functions for Churn modeling (inferred)
└── README.md
